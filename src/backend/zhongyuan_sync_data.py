...
<<<<<<< HEAD
    ...
    # 初始化新日志文件（创建空文件）
    os.makedirs("sync_logs", exist_ok=True)  # 确保日志目录存在
    try:
        with open(current_log_file, 'w', encoding='utf-8') as f:
            f.write(f"===== 同步任务日志 - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} =====\n")
        # 仅写入文件，不再终端打印
        with open(current_log_file, 'a', encoding='utf-8') as f:
            f.write(f"已创建新日志文件：{current_log_file}\n")
    except Exception as e:
        # 日志初始化失败时写入应急日志
        write_emergency_log(f"日志文件初始化失败：{str(e)} | 目标文件：{current_log_file}")


"""写入应急日志（兜底用）"""


def write_emergency_log(content: str):
    try:
        os.makedirs("sync_logs", exist_ok=True)
        emergency_log = "sync_logs/emergency_log.txt"
        with open(emergency_log, 'a', encoding='utf-8') as f:
            f.write(f"【{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}】{content}\n")
    except Exception as e:
        # 终极兜底：打印到终端（防止日志完全丢失）
        native_print(f"应急日志写入失败：{str(e)} | 原始内容：{content}")


"""自定义打印函数：仅输出到当前日志文件（不再输出到终端）"""
...
def log_print(*args, **kwargs):
    ...
    sep = kwargs.get('sep', ' ')
    log_content = f"[{timestamp}] {sep.join(map(str, args))}{end}"
...
<<<<<<< HEAD
        # 日志写入失败时写入应急日志
        write_emergency_log(f"【日志写入失败】：{str(e)} | 内容：{log_content}")
...
def log_exception(exc: Exception, context: str = ""):
    ...
    log_print(error_msg)
    # 4. 同时写入应急日志（双重保障）
    write_emergency_log(f"【程序报错】{context} | 异常类型：{type(exc).__name__} | 详情：{str(exc)}")


# 替换系统print为自定义log_print
print = log_print
    ...
    try:
        while True:
            ...
            try:
                ...
                log_exception(Exception(f"第{current_page}页请求超时"), "获取组织数据超时")
                break
            except requests.exceptions.HTTPError as e:
                log_exception(e, f"第{current_page}页HTTP错误")
                break
            except Exception as e:
                log_exception(e, f"第{current_page}页请求异常")
                break
...
<<<<<<< HEAD
        log_exception(e, f"分组「{group_name}」创建失败")
        return None
...
<<<<<<< HEAD
        log_exception(e, f"组织 {org_info['idt_org__id']} 存入数据库失败")
            ...
            if group_id:
                ...
                if org.get("idt_org__org_code") == "gs1":
                    try:
                        ...
                        log_exception(e, f"创建gs1组织全局默认角色失败")
            else:
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                log_exception(Exception(f"第{current_page}页请求超时"), "获取用户数据超时")
                break
            except requests.exceptions.HTTPError as e:
                log_exception(e, f"第{current_page}页HTTP错误")
                break
...
<<<<<<< HEAD
    try:
        ...
        try:
            ...
            log_exception(e, "读取zhongyuan_role表失败")
            return
        ...
        global_role_id = None
        try:
            resp = await get_group_roles(group_id=[group_id], user=login_user, page=0, limit=0, keyword="普通用户")
            if resp.status_code == 200:
                global_role_id = resp.data['data'][0].id
                print(f"获取到的全局role_id：{global_role_id}")  # 输出 4203
            else:
                raise Exception(f"获取分组ID：{group_id}下的角色ID失败，响应码：{resp.status_code}")
        except Exception as e:
            log_exception(e, f"获取分组ID：{group_id}下的角色ID失败")
            return
        # 5. 循环处理用户数据
        ...
        for user in all_user_data:
            try:
                        ...
                        if not bisheng_user_id:
                            ...
                            try:
                                ...
                                if update_resp.status_code == 200:
                                    ...
                                    raise Exception(f"状态更新失败，响应码 {update_resp.status_code}")
                            except Exception as e:
                                log_exception(e, f"用户 {user_id} 状态更新异常")
                                fail_count += 1
                                fail_users.append(f"{user_account_name}（ID：{user_id}）- {str(e)[:50]}")
                ...
                if global_role_id:
                    ...
                    req = CreateUserReq(user_name=f"{user_name}", password=pub_str,
                                        group_roles=[group_and_roles_1, group_and_roles_2])
                    MAX_RETRIES = 3
                    user_create_success = False
                    created_user = None
                    for i in range(MAX_RETRIES):
                        try:
                            ...
                            log_exception(e, f"创建用户失败（重试{i+1}次）")
                ...
                if user_status == 0 and bisheng_user_id:
                    ...
                    try:
                        ...
                        if update_resp.status_code == 200:
                            ...
                            raise Exception(f"禁用用户失败，响应码 {update_resp.status_code}")
                    except Exception as e:
                        log_exception(e, f"禁用用户异常（用户已创建）")
                ...
                log_exception(e, f"处理用户 {user.get('app_account__id', '未知ID')} 失败")
                fail_count += 1
                fail_users.append(f"{user.get('app_account__account_name', '未知用户')}（ID：{user.get('app_account__id', '未知ID')}）- {str(e)[:50]}")
...
def parse_cli_args():
    ...
    try:
        ...
        # 参数正确，写入日志
        generate_new_log_file()  # 提前创建日志文件
        print(f"定时任务已启动，每天凌晨 {target_hour} 点执行...")
        return target_hour
    except ValueError as e:
        # 参数错误写入应急日志
        error_msg = f"参数错误：{e} | 正确用法：python main.py <小时数> 示例：python main.py 2"
        write_emergency_log(error_msg)
        native_print(error_msg)  # 终端也打印，方便调试
        sys.exit(1)
    except Exception as e:
        # 其他异常
        log_exception(e, "解析命令行参数失败")
        write_emergency_log(f"解析命令行参数异常：{str(e)}")
        sys.exit(1)
...
if __name__ == "__main__":
    try:
        ...
        # 捕获主程序的所有异常
        log_exception(e, "主程序执行异常")
        write_emergency_log(f"主程序崩溃：{str(e)}")
        sys.exit(1)
=======
            print(f"任务执行出错: {str(e)}，1分钟后重新计算调度时间")
            await asyncio.sleep(60)  # 出错后避免频繁重试
